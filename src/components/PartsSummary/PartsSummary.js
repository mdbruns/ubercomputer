...
function partsSummary(props) {
    ...
    if (props.history.location.pathname === "/checkout") {
        for (let category in props.selectedParts) {
            summaryContent.push(
                <div key={shortid.generate()} className={classes.SummaryRow}>
                    <p className={classes.Text}><span className={classes.Category}>{props.headings[category]}</span> : <span>{props.selectedParts[category].name}</span></p>
        ...
        for (let category in props.parts) {
            summaryContent.push(
                <div key={shortid.generate()} className={classes.SummaryRow}>
                    <p className={classes.Text}><span className={classes.Category}>{props.headings[category]}</span> : <span>{props.parts[category] ? props.parts[category].name : "None"}</span></p>
